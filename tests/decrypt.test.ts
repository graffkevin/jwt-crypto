...
import { signToken } from '../src/main';
...
describe('decodeVerifyToken tests', () => {
    it('should decode a token and verify the signature', async () => {
        ...
        const payload = await signToken(basicEncodedJwt, secretKey);
    ...
    it('should throw an error for invalid token format', async () => {
        ...
        await expect(signToken(invalidToken, secretKey)).rejects.toThrow( "The string to be decoded contains invalid characters.");
    });
    ...
    it('should throw an error for invalid token signature', async () => {
        ...
        await expect(signToken(invalidSignatureToken, secretKey)).rejects.toThrow('Invalid signature');
    });
});
